...
import { applyBrowseDelta } from "./applyBrowseDelta";
...
function browse(items: GalleryItem[]): BrowseResponse {
  return {
...
function delta(overrides: Partial<BrowseChangesResponse> = {}): BrowseChangesResponse {
  return { full: false, fingerprint: "after", changed: [], removed: [], ...overrides };
}

describe("applyBrowseDelta", () => {
  it("replaces a changed item without moving it", () => {
    const current = browse([item("a.png"), item("b.png"), item("c.png")]);
    ...
    const next = applyBrowseDelta(current, delta({ changed: [captioned] }));
  ...
  it("drops removed items and keeps the count honest", () => {
    const current = browse([item("a.png"), item("b.png")]);

    const next = applyBrowseDelta(current, delta({ removed: ["C:\\datasets\\sample\\a.png"] }));
  ...
  it("appends an item it has not seen before", () => {
    const current = browse([item("a.png")]);

    const next = applyBrowseDelta(current, delta({ changed: [item("b.png")] }));
  ...
  it("keeps the same object when nothing moved, so React can skip the render", () => {
    ...
    expect(applyBrowseDelta(current, delta({ fingerprint: "before" }))).toBe(current);
  });

  it("still advances the fingerprint when only unlisted entries changed", () => {
    const current = browse([item("a.png")]);

    const next = applyBrowseDelta(current, delta());
